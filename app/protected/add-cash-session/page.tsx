...
import {
  getUserGameTypes,
  getUserLocations,
  getUserCashSessionByID,
  getUserStakes,
} from '@/api/api'
import { FormMessage, Message } from '@/components/shared/form-message'

export default async function AddCashSession(props: {
  ...
  if ('cash_session' in searchParams) {
    try {
      userSession = await getUserCashSessionByID(
        user.id,
        searchParams.cash_session
      )
    } catch (err) {
